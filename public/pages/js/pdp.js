function getDataByWindowUrlKey(){
    ...
    let url = baseUrl+'/api/drone/'+windowUrlKey;
    ...
    .then(function (response) {
      ...
      $('#product-name').html(response.data.drone_name);
      $('#product-producer').html(response.data.producer);
      $('#product-publication').html(response.data.publication_year);
      ...
      // START -- note, unless you have these data in a database structure, here we are hardcoding them for display purposes
        ...
        // tag
        let collectionOfTag = ['Drone','Best Seller','Luxury','Customized','Photography','Farming','Easy Control','High Quality'];
